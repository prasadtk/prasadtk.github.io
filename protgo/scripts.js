...
document.getElementById('searchForm').addEventListener('submit', async function(e) {
  ...
  resultsBody.innerHTML = `<tr><td colspan="6" class="text-center"><span class="spinner"></span> Searching...</td></tr>`;
  ...
  <div class="d-flex justify-content-between align-items-center">
   ...
   <button class="btn btn-sm btn-green ms-2" 
            onclick="checkPDB('${pdbId}', '${detailsId}')">
      More Info
    </button>
  </div>
  <div id="${detailsId}" class="small-text mt-1"></div>
      ...
      const domainHtml = domainFeatures.length
        ? domainFeatures.map(d => `<div>${d}</div>`).join("")
        : `<span class="muted">No domain annotations</span>`;

      // ====== Combined Sequence, Length & Domains column ======
      const seqDomainCol = `
        <div>
          <button class="btn btn-sm btn-sequence w-100 w-md-auto mb-1" onclick="showSequence('${uniId}')">View Sequence</button>
          <div class="small-text">Length: ${seqLength} aa</div>
        </div>
        <div style="margin-top:1em;">${domainHtml}</div>
      `;
    ...
    if (!(data.results || []).length) {
      resultsBody.innerHTML = `<tr><td colspan="6" class="text-center muted">No results found.</td></tr>`;
    }
  } catch {
    resultsBody.innerHTML = `<tr><td colspan="6" class="text-danger">Error fetching results. Please try again.</td></tr>`;
  }
});
...
// ====== Fetch PDB title + DOI + PubMed from RCSB ======
async function checkPDB(pdbId, targetId) {
  const target = document.getElementById(targetId);
  if (!target) return;
  target.innerHTML = `Fetching info <span class="spinner"></span>`;
  ...
  try {
    ...
    if (citation.pdbx_database_id_doi) {
      html += `<div><strong>DOI:</strong> 
        <a class="link" href="https://doi.org/${citation.pdbx_database_id_doi}" target="_blank">
    ...
    if (citation.pdbx_database_id_pubmed) {
      html += `<div><strong>PubMed:</strong> 
        <a class="link" href="https://pubmed.ncbi.nlm.nih.gov/${citation.pdbx_database_id_pubmed}" target="_blank">
    ...
    target.innerHTML = `<span class="text-danger">Failed to fetch info for ${pdbId}.</span>`;
  }
}
